...
import utils
from classify import object_classify
obj_class = object_classify()
...
class VehicleCounter(PipelineProcessor):
    ...
    def __init__(self, exit_masks=[], path_size=10, max_dst=50, x_weight=1.0, y_weight=1.0):
    ...
    def __call__(self, context):
        ...
        context['out_count'] = self.outgoing_count
        context['in_count'] = self.incoming_count
            ...
            for path in self.pathes:
                ...
                for p in points:
                    if len(path) == 1:
                        ...
                        print 'Distance between',path[-1][1],' and ',p[1],' : ',d
                    else:
                        ...
                        print 'Predicted Distance between',(xn,yn),'and',p[1],' : ',d
                    if d < _min:
                        _min = d
                        _match = p
        ...
        for i, path in enumerate(self.pathes):
            ...
            if (
                ...
                print "d[0][1] :", d[0][1],"d[1][1] : ",d[1][1]
                if d[0][1][0] >= 120 and d[0][1][0] <=1280:
                    self.incoming_count += 1
                    print 'Incoming VEHICLES FOUND:',self.incoming_count
                # if d[0][1][0] >= 0 and d[0][1][0] <=645:
                #     self.outgoing_count += 1
                #     print 'Outgoing VEHICLES FOUND:',self.outgoing_count
                self.vehicle_count = self.outgoing_count + self.incoming_count 
                print '+++++++++++++++++++++++++++++++++'
...
class Visualizer(PipelineProcessor):
    ...
    def draw_ui(self, img, vehicle_count, incoming_count, outgoing_count, exit_masks=[]):
        ...
        cv2.putText(img, ("Incoming Vehicles: {incoming} ".format(incoming=incoming_count)), (300, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
        cv2.putText(img, ("Outgoing Vehicles: {Outgoing} ".format(Outgoing=outgoing_count)), (700, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
        #cv2.imshow('img',img)
        return img
    ...
    def __call__(self, context):
        ...
        incoming_count = context['in_count']
        outgoing_count = context['out_count']

        frame = self.draw_ui(frame, vehicle_count, incoming_count, outgoing_count, exit_masks)
